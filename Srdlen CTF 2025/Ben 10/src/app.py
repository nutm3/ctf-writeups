...
def get_user_by_username(username): # hanya ambil username
...
def get_reset_token_for_user(username):  # hanya ambil reset token
...
def update_reset_token(username, reset_token): # buat reset token 
...
def update_password(username, new_password): # update password & reset token null
...
@app.route('/')
def index(): # jika username ada di cookie maka ke /home jika tidak ke login
...
def register(): # if username.startswith('admin') or '^' in username: SUS 1
